...
import { HyperspaceOverlay } from "@/components/hyperspace-overlay";
...
export default function LoginPage() {
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      if (res?.url) {
        redirectUrlRef.current = res.url;
      }
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-black overflow-hidden">
      <GalaxyBackground />
      <HyperspaceOverlay active={hyperspace} onComplete={handleHyperspaceComplete} />

      <div className={`relative z-10 w-full max-w-sm mx-4 transition-opacity duration-500 ${hyperspace ? "opacity-0" : "opacity-100"}`}>
        <div className="rounded-2xl border border-white/[0.08] bg-zinc-950 px-8 pt-10 pb-8">
          {/* Logo */}
          ...
          <button
            ...
            disabled={hyperspace}
            className="w-full flex items-center justify-center gap-2.5 h-11 rounded-xl bg-white text-zinc-900 font-medium text-sm transition-colors hover:bg-zinc-100 active:scale-[0.98] disabled:opacity-50"
          >
